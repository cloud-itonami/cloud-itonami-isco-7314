(ns potterycoord.actor
  "PotteryCoordActor — the ISCO-08 7314 potters and related workers
  pottery workshop scheduling/logistics coordination actor as a
  `langgraph.graph/state-graph` (ADR-2607121000 / CLAUDE.md Actors
  section). One graph run = one pottery-workshop-coordination
  ...
  so an interrupted run can resume after human sign-off. Modeled
  closely on cloud-itonami-isco-7211's foundrycoord.actor (same
  ...
  The unconditional invariant: the Pottery Coordination Advisor can
  never directly commit an operating record the PotteryCoordGovernor
  refuses, and can never make this actor finalize a
  firing/glazing-execution decision (e.g. deciding to proceed with a
  specific kiln firing) or override a workshop safety officer's
  judgment — every commit-record! call is gated behind `:decide`, and
  that decision class is a permanent hard block regardless of
  disposition path. This actor coordinates POTTERY WORKSHOP
  SCHEDULING/LOGISTICS ONLY — it never performs pottery-making,
  glazing or firing work or authorizes kiln-firing operations
  itself."
...
(defn build-graph
  "Build a compiled PotteryCoordActor graph. `store` implements
  `potterycoord.store/Store`. `advisor` implements
  `potterycoord.advisor/Advisor` (defaults to `mock-advisor`).
  ...
  (-> (g/state-graph
      ...
      (g/add-node :commit
                   (fn [{:keys [request proposal]}]
                     (let [record {:potter-id (:potter-id request)
                                    :op (:op proposal)
